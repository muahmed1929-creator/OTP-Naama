...
export const LogTable = ({ logs, onViewAll, onRefresh, refreshing }: LogTableProps) => {
  return (
        ...
        <div className="flex items-center gap-4">
          {onRefresh && (
            <button 
              onClick={onRefresh} 
              disabled={refreshing}
              className="p-2 text-gray-400 hover:text-gray-900 transition-colors rounded-lg hover:bg-gray-100 disabled:opacity-50"
            >
            ...
            {logs.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-8 py-12 text-center text-gray-400 text-sm italic">
                  No logs yet. Generate your first OTP to see data here.
                </td>
              </tr>
            ) : (
              logs.map((log) => (
